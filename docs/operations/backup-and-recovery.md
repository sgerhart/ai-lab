...
- A backup is not valid until a restore test has been performed onto a non-live target.
- `scripts/restore.sh` requires `--confirm-restore` and a backup id. It will not overwrite live volumes without that.
